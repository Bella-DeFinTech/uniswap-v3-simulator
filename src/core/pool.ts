...
import { Tick, Position } from '../model/pool';
...
export class CorePool {
    ...
    constructor(token0: string, token1: string, fee: number, tickSpacing: number,
        token0Balance: BN = new BN(0), token1Balance: BN = new BN(0),
        sqrtPriceX96: BN = new BN(0), tickCurrent: number = 0,
        feeGrowthGlobal0X128: BN = new BN(0), feeGrowthGlobal1X128: BN = new BN(0),
        tickManager: TickManager = new TickManager, positionManager: PositionManager = new PositionManager) {
        this.token0 = token0;
        this.token1 = token1;
    ...
    private modifyPosition(tickLower: number, tickUpper: number, liquidityDelta: BN): { amount0: BN, amount1: BN } {
        // TODO
        return { amount0: new BN(0), amount1: new BN(0) }
    }

    private updatePosition(tickLower: number, tickUpper: number, liquidityDelta: BN) {
        // TODO
    }
